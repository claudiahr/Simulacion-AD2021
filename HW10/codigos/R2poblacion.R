#Regla 2: el valor de cada objeto se genera independientemente con una distribuci???n exponencial y su peso es inversamente correlacionado con el valor. 
...
ggplot(data$`30`, aes(x= Segundo, y= Mejor)) + 
  geom_boxplot(fill = "#77E68A")+
  labs(x = "Tiempo (segs)", y = "Mayor valor", title = 'Soluciones = 30')+
  geom_hline(aes(yintercept=Optimo), colour="#23631F", size= 1)

ggplot(data$`40`, aes(x= Segundo, y= Mejor)) + 
  geom_boxplot(fill = "#77E68A")+
  labs(x = "Tiempo (segs)", y = "Mayor valor", title = 'Soluciones = 40')+
  geom_hline(aes(yintercept=Optimo), colour="#23631F", size= 1)
